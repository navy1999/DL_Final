...
class ODEFunc(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(dim, dim*2), nn.ReLU(), nn.Linear(dim*2, dim))
...
class ODEBlock(nn.Module):
    ...
    def forward(self, z, timepts):
        # if no temporal change, return identity
        if timepts[0] == timepts[-1]: return z
        B, C, H, W, D = z.shape
        z_flat = z.permute(0,2,3,4,1).reshape(-1, C)
        # ensure strictly increasing
        t = timepts.to(z.device)
        if (t[1] <= t[0]): t = t + torch.tensor([0.0, 1e-6], device=t.device)
        out = torchdiffeq.odeint(self.func, z_flat, t, method=self.method,
...
def plot_history(history, save_dir):
    ...
    for key, vals in history.items():
        ...
        plt.savefig(os.path.join(save_dir, f'{key}.png')); plt.close()

# ---------------------- Training & Validation ----------------------
def train_recon(args):
    dl = DataLoader(Brats2023Dataset(args.train_csv), batch_size=args.bs, shuffle=True)
    ...
    for ep in range(args.epochs):
        ...
        for bi,(vol,_,_) in enumerate(dl,1):
            ...
            if not is_pre and bi%args.disc_freq==0:
                with torch.no_grad(): z=vq(enc(vol))[0]; fake=dec(z)
                p_real,p_fake=disc(vol),disc(fake)
                loss_d = torch.mean(nn.functional.relu(1-p_real))+torch.mean(nn.functional.relu(1+p_fake))
                opt_d.zero_grad(); loss_d.backward(); opt_d.step()
                sum_dr+=p_real.mean().item(); sum_df+=p_fake.mean().item()
            z,vl = vq(enc(vol)); recon=dec(z)
            ...
            if args.batch_verbose and bi%args.batch_verbose==0:
                ...
                if not is_pre:
                    c=bi//args.disc_freq; msg+=f" D_r={sum_dr/max(1,c):.4f} D_f={sum_df/max(1,c):.4f}"
                print(msg)
        dr_norm=sum_dr/(len(dl)//args.disc_freq or 1)
        df_norm=sum_df/(len(dl)//args.disc_freq or 1)
        print(f"=> Recon Ep{ep+1}: R={sum_r/len(dl):.4f} VQ={sum_v/len(dl):.4f}"+(f" D_r={dr_norm:.4f} D_f={df_norm:.4f}" if not is_pre else ""))
        if val_dl:
            m=validate(val_dl,enc,dec,vq,ODEBlock(ODEFunc(64)),disc,device,temporal=False,gan=True)
            for k in ['recon','vq','d_real','d_fake']:
                history[f'val_{k}'].append(m[k]); writer.add_scalar(f'Val/{k.capitalize()}',m[k],ep)
        history['train_recon'].append(sum_r/len(dl)); history['train_vq'].append(sum_v/len(dl))
        history['train_d_real'].append(dr_norm); history['train_d_fake'].append(df_norm)
        ...
        if (ep+1)%args.save_ckpt_freq==0:
            ckpt={'epoch':ep+1,'enc':enc.state_dict(),'dec':dec.state_dict(),'vq':vq.state_dict(),
                  'disc':disc.state_dict(),'opt_g':opt_g.state_dict(),'opt_d':opt_d.state_dict()}
            torch.save(ckpt,os.path.join(args.save_dir,f'checkpoint_recon_ep{ep+1}.pth'))
    writer.close(); plot_history(history,args.save_dir)

def train_temporal(args):
    dl=DataLoader(LumiereDataset(args.train_csv),batch_size=args.bs,shuffle=True)
    ...
    disc=Discriminator3D().to(device)
    emb_dim=64
    func=ODEFunc(emb_dim).to(device)
    ...
    for ep in range(args.epochs):
            ...
            if not is_pre and bi%args.disc_freq==0:
                with torch.no_grad():
                    ...
                    for i in range(z.shape[0]):
                        zi=z[i:i+1]
                        t_i=tp[i].item()
                        tpts_i=torch.tensor([0.0, t_i],device=device)
                        lat_list.append(odeb(zi,tpts_i))
                    fake=dec(torch.cat(lat_list,0))
                pr,pf=disc(vol),disc(fake)
                loss_d=torch.mean(nn.functional.relu(1-pr))+torch.mean(nn.functional.relu(1+pf))
                opt_d.zero_grad();loss_d.backward();opt_d.step()
                sum_dr+=pr.mean().item();sum_df+=pf.mean().item()
            z,vl=vq(enc(vol))
            lat_list=[]
            for i in range(z.shape[0]):
                zi=z[i:i+1]; t_i=tp[i].item()
                tpts_i=torch.tensor([0.0,t_i],device=device)
                lat_list.append(odeb(zi,tpts_i))
            lat=torch.cat(lat_list,0)
            recon=dec(lat)
            rl=nn.functional.mse_loss(recon,vol)
        ...
        dr_norm=sum_dr/(len(dl)//args.disc_freq or 1)
        df_norm=sum_df/(len(dl)//args.disc_freq or 1)
        print(f"=> Temporal Ep{ep+1}: R={sum_r/len(dl):.4f} VQ={sum_v/len(dl):.4f} P={sum_p/len(dl):.4f}"+(f" D_r={dr_norm:.4f} D_f={df_norm:.4f}" if not is_pre else ""))
        if val_dl:
            m=validate(val_dl,enc,dec,vq,odeb,disc,device,temporal=True,gan=True)
            for k in ['recon','vq','phys','d_real','d_fake']:
                history[f'val_{k}'].append(m[k]);writer.add_scalar(f'Val/{k.capitalize()}',m[k],ep)
        history['train_recon'].append(sum_r/len(dl));history['train_vq'].append(sum_v/len(dl));history['train_phys'].append(sum_p/len(dl))
        history['train_d_real'].append(dr_norm);history['train_d_fake'].append(df_norm)
        writer.add_scalar('Train/Reconstruction',history['train_recon'][-1],ep)
        writer.add_scalar('Train/VQ',history['train_vq'][-1],ep)
        ...
        if (ep+1)%args.save_ckpt_freq==0:
            ckpt={'epoch':ep+1,'enc':enc.state_dict(),'dec':dec.state_dict(),'vq':vq.state_dict(),
                  'disc':disc.state_dict(),'odefunc':func.state_dict(),'opt_g':opt_g.state_dict(),'opt_d':opt_d.state_dict()}
            torch.save(ckpt,os.path.join(args.save_dir,f'checkpoint_temp_ep{ep+1}.pth'))
    writer.close();plot_history(history,args.save_dir)
...
def validate(dataloader, encoder, decoder, vq, odeblock, discriminator, device, temporal=False, gan=False):
    ...
    metrics=dict.fromkeys(['recon','vq','phys','d_real','d_fake'],0.0);cnt=0
    with torch.no_grad():
        for vol,info,tp in dataloader:
            ...
            if temporal:
                lat_list=[]
                for i in range(z.shape[0]):
                    zi=z[i:i+1];t_i=tp[i].item();tpts_i=torch.tensor([0.0,t_i],device=device)
                    lat_list.append(odeblock(zi,tpts_i))
                q=torch.cat(lat_list,0)
            ...
            metrics['vq']+=vq(encoder(vol))[1].item()
            if gan:
...
if __name__=='__main__':
    ...
    if args.mode=='train_recon': train_recon(args)
    else: train_temporal(args)
